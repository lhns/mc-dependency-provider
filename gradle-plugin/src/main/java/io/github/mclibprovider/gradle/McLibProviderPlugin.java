 ...
 *       served through mc-lib-provider's per-mod classloader at runtime. Anything declared here
 ...
 *   <li>{@code generateMcLibManifest} — walks the resolved {@code mcLibImplementation} closure,
 ...
 *       writes {@code META-INF/mclibprovider.toml} into {@code processResources} output.</li>
 *   <li>{@code prepareMcLibDevCache} — hard-links the resolved jars into the shared cache so dev
...
public final class McLibProviderPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        project.getPluginManager().withPlugin("java", applied -> {
            ...
            // served by mc-lib-provider here. Extends nothing; carries no transitive contributions
            ...
            var generate = project.getTasks().register(
                    "generateMcLibManifest", GenerateMcLibManifestTask.class, t -> {
                        ...
                        t.setDescription("Emits META-INF/mclibprovider.toml from mcLibImplementation.");
                        ...
                        t.getArtifactCoords().set(project.provider(() ->
                                GenerateMcLibManifestTask.collectArtifactCoords(mcLibManifest)));
                        ...
                        t.getPlatformProvidedKeys().set(project.provider(() -> {
                            ...
                            for (var coord : GenerateMcLibManifestTask.collectArtifactCoords(mcLibManifest)) {
                                mclibKeys.add(coord.group() + ":" + coord.name());
                            }
                            Set<String> platformKeys = new HashSet<>();
                            for (var coord : GenerateMcLibManifestTask.collectArtifactCoords(platformView)) {
                                String key = coord.group() + ":" + coord.name();
                                if (!mclibKeys.contains(key)) platformKeys.add(key);
                            }
                            // Always exclude mc-lib-provider's own artifacts — they're shipped by
                            // the platform adapter jar, not via the manifest.
                            platformKeys.add("io.github.mclibprovider:core");
                            platformKeys.add("io.github.mclibprovider:deps-lib");
                            platformKeys.add("io.github.mclibprovider:fabric");
                            platformKeys.add("io.github.mclibprovider:neoforge");
