...
- **Defer `ModClassLoader` construction to `ModLoadingStage.VALIDATE`,** collecting manifests in the constructor and building every loader in one pass. This is the design ADR-0027's "free two-pass" invites and needs no reflection at all. Rejected because it solves only half the problem and breaks the other half: VALIDATE runs *long* after `Bootstrap.bootStrap()`, so the mixin bridge would still be broken — and the lazy populator, which must run during bootstrap, would have nothing to populate. Since the bridge fix forces a `LoadingModList` walk regardless, taking the manifest union from the same walk costs nothing extra.
...
- **Detect mcdp mods by `IModFile.getLoaders()`.** Rejected: it is populated during FML's language-matching phase, which is not obviously complete when the populator fires during bootstrap. `requiredLanguageLoaders()` comes straight out of `mods.toml` parsing and exists as soon as the `IModFileInfo` does.
- **Move `LoggingProgressListener` into `core` and share one copy.** Rejected: the Fabric/NeoForge copies are slf4j-based, the Forge one is JUL-based, and `core` is deliberately logging-framework-free (which is why `ProgressListener` lives in `deps-lib` as a bare interface). Unifying them means picking a logging API for `core` — a bigger decision than this ADR should carry.
