...
**Status:** Accepted — resolved via the shadow-plugin upgrade path (option 2 below). The Fabric adapter subproject (`:fabric`, now `:fabric-1.21` after ADR-0023's band split) emits a single classifier-less jar containing `core` + `deps-lib` classes; composite substitution resolves to that one jar, so Fabric's `ClasspathModCandidateFinder` no longer sees sibling classpath entries. (The jar no longer bundles `tomlj` — ADR-0015 replaced it with the in-tree `MiniToml`.) "Known issue 3" below is **resolved** by [ADR-0014](0014-opt-in-mcdep-implementation.md).
...
3. **`runtimeClasspath` over-captures in Loom projects.** Loom merges the entire MC runtime classpath (guava, gson, icu4j, oshi, mixinextras, Loom's own remapped `mclibprovider:fabric`) into `runtimeClasspath`. `GenerateMcLibManifestTask` faithfully writes all of them to the manifest, where `ManifestConsumer` then fails trying to download them from `libraries.minecraft.net` / the loom-cache's pseudo-repo URL. **Resolved by [ADR-0014](0014-opt-in-mcdep-implementation.md):** the plugin no longer walks `runtimeClasspath` at all — the manifest is resolved from the opt-in `mcdepImplementation` bucket (via `mcdepManifest`), which carries no platform-provided contributions, so Loom/MDG jars can never reach the manifest. Was not dev-only (it affected production manifests too whenever the mod author declared a `modImplementation`).
