...
The pipeline runs as `:generateMcdpBridges` (named `:generateMcdpMixinBridges` when this ADR was written — see the rename table in Errata), between `compileJava/compileScala/compileKotlin` and the `Jar` task:
...
2. **Scan each mixin's bytecode** with `BridgeScanner`:
   ...
   - Walk every method body, classify each instruction against a default-bridge policy (anything not on the platform-prefixes allowlist or in user `sharedPackages` gets bridged). Supported: `INVOKESTATIC`/`INVOKEVIRTUAL`/`INVOKEINTERFACE`/`GETSTATIC`/`GETFIELD`/`PUTSTATIC`/`PUTFIELD`/LDC-class, plus the `NEW T / DUP / args / INVOKESPECIAL T.<init>` constructor triplet. `CHECKCAST`/`INSTANCEOF`/`ANEWARRAY` of mod-private types and `Class.forName` on a mod-private string are formal bridge-model limits — the bridge surface cannot mention mod-private types in any signature, so the rewrite cannot reach them; the scanner emits a warning pointing at `docs/bridges.md` for the workarounds (`sharedPackages` or refactor).
3. **Emit one bridge interface per unique target** (`<bridgePackage>.<TargetSimpleName>_<hash>Bridge`, where `<hash>` is the first 6 hex of the SHA-256 of the target's fully-qualified name — the bare simple name collided across packages) as bytecode, holding one method per (member-name, descriptor) tuple. Static methods keep their descriptor; virtual methods get a leading receiver parameter; field reads become getter methods.
...
5. **Rewrite the mixin's bytecode** (`BridgeRewriter`): add a synthetic `LOGIC_<TargetSimpleName>_<hash>` static field per target (same collision-free naming as the interface), and replace each cross-classloader call site with a stack-juggle sequence (spill args to fresh locals, push the LOGIC field, reload locals, INVOKEINTERFACE the bridge). `LineNumberTable` is preserved by walking the existing `InsnList` rather than rebuilding the method.
...
A later pass finished the same cleanup on the plugin's own internals, which are not consumer-visible: package `de.lhns.mcdp.gradle.mixinbridges` → `de.lhns.mcdp.gradle.bridges`, `BridgeMixinScanner` → `BridgeScanner`, `MixinScanResult` → `BridgeScanResult`, `MixinRewriter` → `BridgeRewriter` (tests renamed to match). References to those paths elsewhere in this ADR and in 0021/0022/0024 were updated in place so they resolve; nothing about the decisions themselves changed. Genuinely Mixin-specific names were left alone: `McdpMixin`, `MixinConfigScanner` (it really does parse `*.mixins.json`), the `mixin` TOML key, and `mixinConfigs`.
