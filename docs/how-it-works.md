...
| `gradlew build` — `:generateMcdpBridges` | Gradle JVM | ASM-walk every `.class` in `main.output.classesDirs` for class-level annotations matching `bridgedAnnotations`. For each match: scan bytecode → rewrite cross-classloader call sites → emit bridge interface + impl + manifest entries. Lambda sites (INVOKEDYNAMIC LambdaMetafactory) rewritten too. Rewritten classes are written back over the input file (in-place over `build/classes/<lang>/main/`); new classes (interfaces/impls/wrappers) go to `build/mcdp-bridges/classes/`. See ADR-0018 §"Errata: rewrite in place vs. layered copy" for the trade-off. |
