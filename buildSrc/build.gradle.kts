...
// that `plugins { id(...) }` would resolve.
dependencies {
    implementation(libs.plugins.shadow.map { "${it.pluginId}:${it.pluginId}.gradle.plugin:${it.version}" })
    implementation(libs.plugins.vanniktech.maven.publish.map { "${it.pluginId}:${it.pluginId}.gradle.plugin:${it.version}" })
}
