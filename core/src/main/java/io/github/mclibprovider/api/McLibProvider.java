...
/**
 * Public facade for mods. Lives under {@code io.github.mclibprovider.api.*} — a platform-prefix
 ...
 * <p>
 * Populated by the platform adapters (NeoForge {@code McLibLanguageLoader}, Fabric
 * {@code McLibLanguageAdapter}) as mods are discovered.
 */
public final class McLibProvider {

    private McLibProvider() {}
     ...
     * <p>
     * Walks the stack to find the calling class (assumed to be a {@code @McLibMixin}-annotated
     * mixin), reads its {@link McLibMixin#impl()}, looks up the owning mod's
     ...
     * <p>
     * Typical use: {@code private static final MyLogic L = McLibProvider.loadMixinImpl(MyLogic.class);}
     */
    ...
    /**
     * Find the first caller outside {@link McLibProvider} itself. Uses {@link StackWalker} with
    ...
    private static Class<?> callerClass() {
        return StackWalker.getInstance(StackWalker.Option.RETAIN_CLASS_REFERENCE)
                .walk(frames -> frames
                        ...
                        .filter(c -> c != McLibProvider.class)
                        .findFirst()
                        .orElse(null));
    }
